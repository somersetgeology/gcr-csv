...
function csv2geojson(x, options, callback) {
    ...
    var latfield = options.latfield || '',
        ...
        eastingfield = options.eastingfield || '',
        northingfield = options.northingfield || '',
        bngfield = options.bngfield || '';
        shortbngfield = options.shortbngfield || ''; 
    ...
    if (!bngfield) {
        for (var f in parsed[0]) {
            if (!bngfield && isBng(f)) bngfield = f;
        }
    }
    ...
    for (var i = 0; i < parsed.length; i++) {
		if (parsed[i][bngfield] !== undefined) { 

            var bngf = parsed[i][bngfield];
                bngf = String(bngf).trim();
                // validate format
                match = bngf.match(/^[A-Z]{2}\s*[0-9]+\s*[0-9]+$/i);
                if (!match) throw new Error(`invalid grid reference ‘${bngf}’`);
                // get numeric values of letter references, mapping A->0, B->1, C->2, etc:
                let l1 = bngf.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0);
                let l2 = bngf.toUpperCase().charCodeAt(1) - 'A'.charCodeAt(0);
                // shuffle down letters after 'I' since 'I' is not used in grid:
                if (l1 > 7) l1--;
                if (l2 > 7) l2--;
                // sanity check
                if (l1<8 || l1 > 18) throw new Error(`invalid grid reference ‘${bngf}’`);
               ...
               var en = bngf.slice(2).trim().split(/\s+/);
               // if e/n not whitespace separated, split half way
               if (en.length == 1) en = [ en[0].slice(0, en[0].length / 2), en[0].slice(en[0].length / 2) ];
               
              // validation
              if (en[0].length != en[1].length) throw new Error(`invalid grid reference ‘${bngf}’`);

              // standardise to 10-digit refs (metres)
              en[0] = en[0].padEnd(5, '0');
              en[1] = en[1].padEnd(5, '0');

              eastingf = e100km + en[0];
              northingf = n100km + en[1];              
                
            eastingf = parseFloat(eastingf);
            northingf = parseFloat(northingf);
            ...
            var latlngcoord = proj4(bngproj, 'WGS84', [eastingf, northingf]);
            lonf = latlngcoord[0].toFixed(5);
            latf = latlngcoord[1].toFixed(5);
